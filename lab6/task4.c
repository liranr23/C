...
int **createPipes(int nPipes){
  int** pipes_array = (int**)(malloc(sizeof(int *)*nPipes));
  int i;
  for (i = 0; i < nPipes; i++){
    pipes_array[i] = (int*)(malloc(sizeof(int)*2));
  }
  return pipes_array;
}
...
int *leftPipe(int **pipes, cmdLine *pCmdLine){
  ...
  int counter = pCmdLine->idx;
  if (counter > 0){
    return pipes[counter-1];
  }
  else{
    return 0;
  }
...
void real_execute(cmdLine *pCmdLine, cmdLine *next_cmd, int* fd,  int** pipes, int command_num){
  int* left_Pipe = leftPipe(pipes, pCmdLine);
  int* right_Pipe = rightPipe(pipes, pCmdLine);
  if(right_Pipe != 0){
    close(STDOUT);
    dup(fd[1]);
    close(fd[1]);
  ...
  if(pCmdLine->inputRedirect){
    input = open (pCmdLine->inputRedirect, O_RDONLY, 0777);
    dup2(input, STDIN);
    close(input);
  }
...
void start_execute(cmdLine *pCmdLine, int ** pipes_array, int pipes_size, int command_num){
  int* fd = pipes_array[command_num];
  cmdLine* next_command = pCmdLine->next;
  if(next_command != 0){
    if(pipe(fd) == -1){
      perror("pipe");
      exit(1);
  ...
  if(pid == 0){
    real_execute(pCmdLine, next_command, fd, pipes_array, command_num);
  }
  else if((command_num+1) < pipes_size){
    close(fd[1]);
    start_execute(pCmdLine->next, pipes_array, pipes_size,(command_num+1));
    close(fd[0]);
  }
  if ((command_num+1) == pipes_size){
...
void execute(cmdLine *pCmdLine){
  ...
  while(pointer){
    ...
    cmdLine* pointer2 = pointer->next;
    pointer = pointer2;
  }
  int ** pipes_array = createPipes(num_of_pipes);
  start_execute(pCmdLine, pipes_array, num_of_pipes, pCmdLine->idx);
  releasePipes(pipes_array, num_of_pipes);
}
...
var_list* delete_var(var_list *list, char* name){
  ...
  if(list == 0){
    perror("the list is empty");
  }else if(strcmp(pointer->name, name) == 0){
    deleted = 1;
    list = backtrace->next;
    free(pointer->name);
    free(pointer->value);
